...
compInOut f _ (inp,outp) = let outp' = parse f inp
    in if outp == outp'
    then Right True
    else fail $ "INP " ++ inp ++ " EXPECTED " ++ show outp ++ " GOT " ++ show outp'
...
parseFloatTest =
    let inps  = [     "12.",     "12.3",      "-1." ,      "-1.4" ,     "15.6"
        ...
        outps = [ Just 12.0, Just 12.3 , Just (-1.0), Just (-1.4) , Just 15.6
                ...
                , Just (-1)]
        gentest ("-1.-1",_) = [(Float (-1.0), "-1")]
        gentest ("10..10",_) = [(Float 10.0, ".10")]
        gentest (_,Just n) = [(Float n, "")]
        gentest (_,Nothing) = []
